...
// lib/features/business/types/trip_booking/screens/trip_itinerary_screen.dart
...
class _TripItineraryScreenState extends State<TripItineraryScreen> {
  final List<Map<String, dynamic>> _stops = [
    {
      'id': 1,
      'title': 'Siwa House Museum',
      'time': '09:00 AM',
      'duration': '1 hour',
      'description': 'Traditional Siwan architecture and culture',
    },
    {
      'id': 2,
      ...
      'time': '11:00 AM',
      'duration': '2 hours',
      'description': 'Natural spring pool swimming',
    },
    {
      'id': 3,
      ...
      'time': '02:00 PM',
      'duration': '1.5 hours',
      'description': 'Ancient tombs and panoramic views',
    },
  ];
            ...
            child: Column(
              children: [
                ListTile(
                  ...
                  title: Text(stop['title'], style: AppTheme.titleMedium),
                  subtitle: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      const SizedBox(height: 4),
                      Row(
                        children: [
                          ...
                          const Icon(Icons.schedule, size: 16, color: AppTheme.gray),
                          const SizedBox(width: 4),
                          Text(stop['duration']),
                        ],
                      ),
                      const SizedBox(height: 4),
                      Text(stop['description'], style: AppTheme.bodySmall),
                    ],
                  ),
                  trailing: Row(
                    mainAxisSize: MainAxisSize.min,
  ...
  void _showEditStopDialog(Map<String, dynamic> stop) {
    ...
    final timeController = TextEditingController(text: stop['time']);
    final durationController = TextEditingController(text: stop['duration']);
          ...
          ElevatedButton(
            onPressed: () {
              setState(() {
                ...
                stop['time'] = timeController.text;
                stop['duration'] = durationController.text;
                stop['description'] = descController.text;
              });
              Navigator.pop(context);
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text('Stop updated')),
              );
            },
            child: const Text('Save'),
          ),
  ...
  void _deleteStop(Map<String, dynamic> stop) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Remove "${stop['title']}" from itinerary?'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
            child: const Text('Cancel'),
