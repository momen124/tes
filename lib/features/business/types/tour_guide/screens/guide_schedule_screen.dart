...
class _GuideScheduleScreenState extends ConsumerState<GuideScheduleScreen> {
          ...
          return Card(
            ...
            child: ListTile(
              ...
              trailing: Column(
                ...
                children: [
                  Text(
                    "\$${((tour['price'] ?? 0) as num).toDouble().toStringAsFixed(0)}",
  ...
  void _showAddTourDialog(DateTime day) {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        title: Text('Add Tour - ${_formatDate(day)}'.tr()),
        content: Form(
          key: formKey,
            ...
            onPressed: () {
              if (formKey.currentState!.validate()) {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text('Tour added successfully'.tr())),
                );
  ...
  void _showEditTourDialog(Map<String, dynamic> tour, DateTime day) {
    ...
    final guestsController = TextEditingController(text: (tour['guests'] ?? 0).toString());
    final priceController = TextEditingController(text: (tour['price'] ?? 0).toString());
